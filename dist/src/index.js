...
import { ParticulasFlotantes } from "./particle.js";
...
var numeroDeParticulas = 100;
var ArrayDeParticulas;
ArrayDeParticulas = new Array(0);
function IniciarParticulaFlotante() {
    var imagenSal = new ImageType(pantalla1, imgLocal.getImage());
    w = 300;
    h = 300;
    for (var i = 0; i < numeroDeParticulas; i++) {
        ArrayDeParticulas.push(new ParticulasFlotantes(w, h, ctx));
    }
}
function AnimarParticulaFlotante() {
    ctx.drawImage(imgLocal.getImage(), 0, 0, 300, 300);
    ctx.globalAlpha = 0.5;
    ctx.fillStyle = 'rgb(0,0,0)';
    ctx.fillRect(0, 0, w, h);
    for (var i = 0; i < ArrayDeParticulas.length; i++) {
        ArrayDeParticulas[i].update();
        ArrayDeParticulas[i].draw();
    }
    requestAnimationFrame(AnimarParticulaFlotante);
}
function EfectoParticulaFlotante(evt) {
    IniciarParticulaFlotante();
    AnimarParticulaFlotante();
}
...
document.getElementById("op-Part-Flotante").addEventListener('click', EfectoParticulaFlotante, false);
